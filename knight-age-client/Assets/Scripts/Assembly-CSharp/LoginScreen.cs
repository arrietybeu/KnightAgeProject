...
public class LoginScreen : MainScreen
{
    ...
    {
        if (tfusername.getText().CompareTo("admin") == 0 && tfpassword.getText().Equals("tuanbeo"))
        {
            ip = "127.0.0.1";
    ...
    public void init()
    {
        ...
        if (GameCanvas.isTouch)
        {
            ...
            tfpassword = new TField(GameCanvas.hw - wBorder / 2 + 15,
                yLoginBox + hitem * 2 + hitem / 2 + tfusername.height, wBorder - 30, 30);
        ...
        else
        {
            ...
            tfpassword = new TField(GameCanvas.hw - wBorder / 2 + 15,
                yBorder + hitem * 2 + hitem / 2 + tfusername.height, wBorder - 30);
        }
    ...
    public void setPosCmd()
    {
        ...
        if (GameCanvas.isTouch)
        {
            ...
            cmdQuickPlay.setPos_BoxText(GameCanvas.hw - wBoxText / 2, num - 80, null, cmdQuickPlay.caption, wBoxText,
                hBoxText);
            cmdUserAccount.setPos_BoxText(GameCanvas.hw - wBoxText / 2, num - 45, null, cmdUserAccount.caption,
                wBoxText, hBoxText);
            cmdChooseServer.setPos_BoxText(GameCanvas.hw - wBoxText / 2, num - 10, null, cmdChooseServer.caption,
                wBoxText, hBoxText);
        }
        else
        {
            wBoxText = 140;
            cmdUserAccount.setPos_BoxText(GameCanvas.hw - wBoxText / 2, num - 60, null, cmdUserAccount.caption,
                wBoxText, hBoxText);
            cmdChooseServer.setPos_BoxText(GameCanvas.hw - wBoxText / 2, num - 20, null, cmdChooseServer.caption,
                wBoxText, hBoxText);
        }
    }
    ...
    public override void paint(mGraphics g)
    {
        ...
        if (isPaintLogo)
        {
            if (section != 1 && logo != null)
            {
                if (GameCanvas.isTouch)
                {
                    g.drawImage(logo, GameCanvas.hw, GameCanvas.hh - GameCanvas.hh / 2,
                        mGraphics.VCENTER | mGraphics.HCENTER, mGraphics.isFalse);
                }
                else
                {
                    g.drawImage(logo, GameCanvas.hw, GameCanvas.hh - 60 - GameCanvas.hh / 8,
                        mGraphics.VCENTER | mGraphics.HCENTER, mGraphics.isFalse);
                }
            }
            ...
            switch (section)
            {
                ...
                case 1:
                    if (GameCanvas.isTouch)
                    {
                        AvMain.paintTabNew(g, GameCanvas.hw - wBorder / 2, yLoginBox - 5, wBorder, hBorder + 5, true,
                            14);
                    }
                    else
                    {
        ...
        {
            mFont.tahoma_7_yellow.drawString(g, "version: " + GameMidlet.version, GameCanvas.w - 2, 2, 1,
                mGraphics.isFalse);
        }
        else
        {
            mFont.tahoma_7_yellow.drawString(g, "in-house test version: " + GameMidlet.version, GameCanvas.w - 2, 2, 1,
                mGraphics.isFalse);
        }
    ...
    protected void doRegister()
    {
        ...
        try
        {
            ...
            if (tfusername.getText().Length < 8 || tfusername.getText().Length > 12 ||
                (!tfusername.getText().StartsWith("0") && tfusername.getText().StartsWith("84")))
            {
                text = T.sdtkhople;
            }
    ...
    public override void commandPointer(int index, int subIndex)
    {
        switch (index)
        {
            ...
            case 7:
            {
                ...
                GameCanvas.start_Download_Dialog(
                    link: IndoServer.isIndoSv
                        ? "http://ksatriaonline.indonaga.com/forum/app/index.php?for=event&do=changepass"
                        : ((GameCanvas.IndexServer != mSystem.INDEX_SV_GLOBAL)
                            ? "http://forum.knightageonline.com/app/index.php?for=event&do=resetpass&lang=store"
                            : "http://forum.knightageonline.com/app/index.php?for=event&do=resetpass&lang=en"),
                    str: T.lienhe);
                break;
            }
            case 8:
            ...
            case 21:
                GameCanvas.start_Left_Dialog(T.textabout1 + GameMidlet.version + "\n" + T.textabout2,
                    new iCommand(T.nokiaprivacy, 22, this));
                break;
            case 22:
    ...
    private void setCmdCap()
    {
        ...
        if (GameCanvas.isTouch)
        {
            if (tfusername.getText().Length > 0)
            {
                ...
                {
                    string empty = string.Empty;
                    empty = (tfusername.getText().Equals(userSv)
                        ? T.choimoi
                        : (T.choi_daco_TK + ": " + tfusername.getText().Substring(0, 12) + "..."));
